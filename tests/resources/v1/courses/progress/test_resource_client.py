...
def test_list_200_generated_success():
    ...
    response = client.v1.courses.progress.list(
        course_id=123, user_id=123, page=123, per=123
    )
    adapter = TypeAdapter(models.CourseProgressesResponse)
...
async def test_await_list_200_generated_success():
    ...
    response = await client.v1.courses.progress.list(
        course_id=123, user_id=123, page=123, per=123
    )
    adapter = TypeAdapter(models.CourseProgressesResponse)
    adapter.validate_python(response)
